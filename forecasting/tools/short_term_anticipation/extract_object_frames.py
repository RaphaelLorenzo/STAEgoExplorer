...
parser.add_argument('--only_local_videos', action='store_true')
...
local_videos = set(os.listdir(args.path_to_videos))
print('Found {} local videos'.format(len(local_videos)))
...
for ann in [train, val, test]:
    for x in ann['annotations']:
        ...
        if args.only_local_videos and not x["video_uid"]+".mp4" in local_videos:
            continue
